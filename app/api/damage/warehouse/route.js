...
export async function POST(req) {
  ...
  try {
    const { manager_ID, employee_ID, damage_date, items, damage_type = "Warehouse" } = await req.json();
    console.log("DAMAGE PAYLOAD:", { manager_ID, employee_ID, damage_date, items, damage_type });
    ...
    for (const item of items) {
      ...
      if (damage_quantity > currentStock) {
        throw new Error(`Damage exceeds available stock (${currentStock}).`);
      }
      ...
      await connection.query(
        `INSERT INTO tbl_damage_withinwarehouse
         (product_id, manager_id, employee_id, damage_quantity, damage_amount, damage_date, damage_subtotal, damage_description, damage_type)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [product_ID, manager_ID, employee_ID, damage_quantity, subtotal, damage_date, subtotal, damage_description, damage_type]
      );

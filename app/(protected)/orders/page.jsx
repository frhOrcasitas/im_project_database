...
function DeliveryDamageModal({ onClose, onSuccess, initialShipmentID = "" }) {
  ...
  const doLoad = async (id) => {
    ...
    try {
      ...
      setError(err.message); setItems([]); setLoaded(false);
    } finally {
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[9999] p-4" style={{ pointerEvents: 'auto' }}>
      <div className="bg-white rounded-2xl w-full max-w-xl shadow-xl max-h-[90vh] flex flex-col overflow-hidden" style={{ pointerEvents: 'auto' }}>
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between shrink-0">
          ...
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 w-8 h-8 flex items-center justify-center rounded-lg hover:bg-slate-100 flex-shrink-0">✕</button>
        </div>
          ...
          {items.length > 0 && (
            <div>
              ...
              <div className="flex flex-col gap-2">
                {items.map((item, idx) => (
                  <div key={idx} onClick={() => updateItem(idx, "include", !item.include)}
                    className={`border rounded-xl p-3 transition-all cursor-pointer hover:shadow-sm ${item.include ? "border-orange-300 bg-orange-50" : "border-slate-200 bg-slate-50 hover:bg-orange-50"}`}>
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-3">
                        <div className={`w-5 h-5 flex items-center justify-center rounded border ${item.include ? "bg-orange-500 border-orange-500 text-white" : "border-slate-300 bg-white"}`}>
                          {item.include && "✓"}
                        </div>
                        <div>
                          <p className="font-semibold text-slate-800">{item.product_name}</p>
                          <p className="text-xs text-slate-500">Shipped: {item.shipped_qty}</p>
                        </div>
                      </div>
                      {item.include && (
                        <input type="number" min="1" max={item.shipped_qty} value={item.damage_quantity}
                          onClick={e => e.stopPropagation()}
                          onChange={e => updateItem(idx, "damage_quantity", parseInt(e.target.value) || 1)}
                    ...
                    {item.include && (
                      <div className="mt-3">
                        <input type="text" placeholder="Optional damage description" value={item.damage_description || ""}
                          onClick={e => e.stopPropagation()}
            ...
            <>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-slate-50">
                      <tr>
                        ...
                        <th className="text-center px-4 py-2.5 text-xs font-bold text-slate-500">Unit</th>
                    ...
                    <tbody>
                      {items.map((item, i) => {
                        const pcsPerCase = parsePcsPerCase(item.product_unit);
                        const pieces = item.salesDetail_unitType === "Cases" && pcsPerCase
                          ? item.salesDetail_qty * pcsPerCase + (item.salesDetail_remainder || 0)
                          : item.salesDetail_qty;
                        return (
                          <tr key={i} className="border-t border-slate-50">
                            <td className="px-4 py-3 font-medium text-slate-700">{item.product_name}</td>
                            <td className="px-4 py-3 text-right text-slate-600">
                              {item.salesDetail_qty} {item.salesDetail_unitType || ""}
                              {item.salesDetail_unitType === "Cases" && item.salesDetail_remainder > 0
                                && ` + ${item.salesDetail_remainder} pcs`}
                            </td>
                            <td className="px-4 py-3 text-center text-xs text-slate-400">
                              {pcsPerCase ? `${pieces} pcs` : "—"}
                            </td>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div>
                  ...
                  <select className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                    value={form.manager_id} onChange={e => setForm(f => ({ ...f, manager_id: e.target.value }))}>
              ...
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wide block mb-2">
                  ...
                  <span className="ml-2 text-slate-400 font-normal normal-case">({form.selectedEmployees.length} selected)</span>
                </label>
                <div className="border border-slate-200 rounded-xl overflow-hidden max-h-48 overflow-y-auto">
                  {employees.filter(e => e.employee_status === "Active").map(emp => {
                    ...
                    return (
                      <button key={emp.employee_ID} type="button" onClick={() => toggleEmployee(emp.employee_ID)}
                        className={`w-full flex items-center gap-3 px-4 py-3 text-left border-b border-slate-50 last:border-0 transition-colors ${selected ? "bg-blue-50" : "hover:bg-slate-50"}`}>
                        <div className={`w-5 h-5 rounded border-2 flex items-center justify-center shrink-0 ${selected ? "bg-blue-600 border-blue-600" : "border-slate-300"}`}>
                        ...
                        {Number(emp.isManager) === 1 && <span className="text-xs text-violet-600 font-semibold">⭐ Manager</span>}
                      </button>
                    );
                  })}
                </div>
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-xl overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          <h2 className="text-lg font-bold text-slate-800">{mode === "add" ? "🚛 Add Vehicle" : "✏️ Edit Vehicle"}</h2>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 w-8 h-8 flex items-center justify-center rounded-lg hover:bg-slate-100">✕</button>
        </div>
        <div className="px-6 py-5 flex flex-col gap-4">
          ...
          <div>
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wide block mb-1.5">Plate / Vehicle Number <span className="text-red-400">*</span></label>
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex gap-3">
          <button onClick={onClose} className="flex-1 border border-slate-200 text-slate-600 font-medium py-2.5 rounded-lg text-sm hover:bg-slate-50">Cancel</button>
          <button onClick={handleSubmit} disabled={submitting}
            className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-semibold py-2.5 rounded-lg text-sm">
...
export default function Orders() {
  ...
  const [damageModalShipID,     setDamageModalShipID]     = useState(null);

  const fetchSales     = async () => { const r = await fetch("/api/sales");    const d = await r.json(); setSales(Array.isArray(d) ? d : []); };
  const fetchVehicles  = async () => { const r = await fetch("/api/vehicle");  const d = await r.json(); setVehicles(Array.isArray(d) ? d : []); };
  const fetchShipments = async () => { const r = await fetch("/api/shipment"); const d = await r.json(); setShipments(Array.isArray(d) ? d : []); };
  ...
  const handleDeleteVehicle = async (vehicle) => {
    ...
    try {
      ...
      if (res.ok) { setToast({ message: "Vehicle deleted.", type: "success" }); fetchVehicles(); }
      else { const d = await res.json(); setToast({ message: d.error || "Delete failed.", type: "error" }); }
    ...
    async function initData() {
      ...
      try {
        const [salesRes, clientRes, productRes, vehicleRes, shipmentRes] = await Promise.all([
          fetch("/api/sales"), fetch("/api/client"), fetch("/api/products"), fetch("/api/vehicle"), fetch("/api/shipment"),
        ]);
  ...
  const handleViewClick = async (order) => {
    setOrderDetails([]); setViewingOrder(order); setIsViewModalOpen(true);
    try {
  ...
  return (
    <div className="p-4 lg:p-8 max-w-7xl mx-auto">
      ...
      <div className="bg-white rounded-xl shadow-sm p-5 border border-slate-100">
          ...
          <div className="flex gap-2 flex-wrap">
            {["all", "Pending", "In Transit", "Completed", "Cancelled"].map(status => (
              <button key={status} onClick={() => setStatusFilter(status)}
                className={`text-xs font-semibold px-3 py-1.5 rounded-lg transition-all ${statusFilter === status ? "bg-slate-800 text-white shadow-md" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>
        ...
        <div className="overflow-x-auto">
          <div className="max-h-[420px] overflow-y-auto">
                  ...
                  <tr key={s.sales_ID} className="hover:bg-slate-50 transition-colors group">
                    ...
                    <td className="py-4 px-3">
                      <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button className="text-[10px] font-bold uppercase bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white px-3 py-1.5 rounded-md transition-all"
                          onClick={() => handleViewClick(s)}>View</button>
                        {s.sales_status === "Pending" && (
                          <button className="text-[10px] font-bold uppercase bg-orange-50 text-orange-600 hover:bg-orange-600 hover:text-white px-3 py-1.5 rounded-md transition-all"
                            onClick={() => setShippingOrder(s)}>Ship</button>
                        )}
                        {s.sales_status === "In Transit" && (
                          <button className="text-[10px] font-bold uppercase bg-green-50 text-green-600 hover:bg-green-600 hover:text-white px-3 py-1.5 rounded-md transition-all"
                            onClick={() => updateOrderStatus(s.sales_ID, "Completed")}>Delivered</button>
                        )}
                        {s.sales_status !== "Cancelled" && s.sales_status !== "Completed" && (
                          <button className="text-[10px] font-bold uppercase bg-red-50 text-red-600 hover:bg-red-600 hover:text-white px-3 py-1.5 rounded-md transition-all"
                            onClick={() => updateOrderStatus(s.sales_ID, "Cancelled")}>Cancel</button>
                        )}
                      </div>
                    </td>
                  </tr>
            ...
            {filtered.length === 0 && <div className="py-12 text-center text-slate-400 italic">No orders found.</div>}
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-5 border border-slate-100 mt-6">
        <div className="flex items-center justify-between mb-5">
          ...
          <button onClick={() => setVehicleModal({ mode: "add" })}
            className="bg-slate-800 hover:bg-slate-900 text-white text-xs font-semibold px-3 py-2 rounded-lg">+ Add Vehicle</button>
        </div>
        {vehicles.length === 0 ? (
              ...
              <div key={v.vehicle_ID} className="border border-slate-200 rounded-xl p-4 hover:border-slate-300 hover:shadow-sm transition-all group">
                <div className="flex items-start justify-between gap-2 mb-3">
                  ...
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => setVehicleModal({ mode: "edit", initial: { ...v } })}
                      className="text-[10px] font-bold bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white px-2 py-1 rounded">Edit</button>
                    <button onClick={() => handleDeleteVehicle(v)}
                      className="text-[10px] font-bold bg-red-50 text-red-500 hover:bg-red-500 hover:text-white px-2 py-1 rounded">Del</button>
                  </div>
                </div>
                {v.vehicle_model && <div className="text-xs text-slate-600 font-medium mb-1">{v.vehicle_model}</div>}
                {v.vehicle_description && <div className="text-xs text-slate-400 leading-relaxed">{v.vehicle_description}</div>}
              </div>
            ))}
          </div>
        )}
      ...
      {vehicleModal && (
        <VehicleModal mode={vehicleModal.mode} initial={vehicleModal.initial}
          onClose={() => setVehicleModal(null)}
          onSuccess={msg => { setVehicleModal(null); setToast({ message: msg, type: "success" }); fetchVehicles(); }} />
      )}
      ...
      <NewSaleModal open={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}
        onSuccess={msg => { setIsCreateModalOpen(false); setToast({ message: msg, type: "success" }); fetchSales(); }}
        clients={clients} products={products} />

      {shippingOrder && (
        <ShipModal order={shippingOrder} onClose={() => setShippingOrder(null)}
          onSuccess={msg => { setShippingOrder(null); setToast({ message: msg, type: "success" }); fetchSales(); fetchShipments(); }} />
      )}
      ...
      <div className="bg-white rounded-xl shadow-sm p-5 border border-slate-100 mt-6">
        ...
        <div className="mb-4">
          <input type="text" placeholder="Search by customer, shipment # or order #..."
            value={shipmentSearch} onChange={e => setShipmentSearch(e.target.value)}
            className="w-full max-w-xs border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-400 bg-white" />
        </div>
        {shipments.length === 0 ? (
                    ...
                    <tr key={sh.shipment_ID} className="hover:bg-slate-50 transition-colors group">
                      ...
                      <td className="py-3 px-3">
                        <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
                          ...
                          "bg-red-100 text-red-700"}`}>{sh.sales_paymentStatus}</span>
                      </td>
                      <td className="py-3 px-3">{statusBadge(sh.sales_status)}</td>
                      <td className="py-3 px-3">
                        <button onClick={() => handleViewShipment(sh)}
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl shadow-xl max-h-[90vh] flex flex-col overflow-hidden">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-slate-50 rounded-xl p-3 border border-slate-100">
                      ...
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
                        ...
                        "bg-red-100 text-red-700"}`}>{shipmentDetail.shipment.sales_paymentStatus}</span>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {shipmentDetail.employees.map((emp, i) => (
                          <div key={i} className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2">
                            <div className="w-6 h-6 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 text-xs font-bold">{emp.employee_name?.charAt(0)}</div>
                            <div>
            ...
            <div className="px-6 py-4 border-t border-slate-100 shrink-0 flex gap-3">
              <button onClick={() => setShipmentDetailOpen(false)}
                className="flex-1 border border-slate-200 text-slate-600 font-medium py-2.5 rounded-lg text-sm hover:bg-slate-50">Close</button>
              {shipmentDetail && (
                <button onClick={() => { setDamageModalShipID(shipmentDetail.shipment.shipment_ID); setShipmentDetailOpen(false); }}
                  className="flex-1 bg-orange-500 hover:bg-orange-600 text-white font-semibold py-2.5 rounded-lg text-sm">
                  ⚠️ Record Damage
                </button>
              )}
            </div>
      ...
      {damageModalShipID !== null && (
        <DeliveryDamageModal initialShipmentID={damageModalShipID}
          onClose={() => setDamageModalShipID(null)}
          onSuccess={msg => { setDamageModalShipID(null); setToast({ message: msg, type: "success" }); }} />
      )}
